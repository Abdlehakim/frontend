...
export default function SignInPage() {
  ...
  const { login, refresh } = useAuth();                 /* ← NEW */
  ...
  const handleGoogleSignIn = async (resp: CredentialResponse) => {
    ...
    try {
      const r = await fetch(`${backendUrl}/api/signin/google`, {
        method: "POST",
  ...
  return (
    <div className="w-flex w-full h-screen items-center">
        ...
        <div className="px-8 flex flex-col w-[600px] h-[700px] bg-white bg-opacity-80 rounded-xl justify-center gap-4 z-10">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-2">
            ...
            <div className="flex flex-col items-center h-20">
              {isGoogleLoading || !hasGoogleLoaded ? (
                ...
                <GoogleLogin onSuccess={handleGoogleSignIn} onError={() => setError("Google sign-in failed")} />
              )}
            </div>
          ...
          <div className="flex gap-4 justify-center">
            {[FaFacebookF, FaInstagram, FaTwitter, FaYoutube].map((Icon, i) => (
              <a key={i} href="#" className="w-12 h-12 border-4 border-gray-500 rounded-full flex items-center justify-center text-gray-500">
      ...
      <div className="fixed inset-0 -z-10">
        <Image src="/signin.jpg" alt="Sign-in background" fill className="object-cover" />
      </div>
    </div>
  );
}
